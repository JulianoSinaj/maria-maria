...
export function validatePatch(slug, patch) {
  ...
  if (patch.asset !== undefined) {
    /* two legal sources: the wine's tracked packshots under public/, or a
       mock upload served through the file route (uploads live outside
       public/ because `next start` snapshots public/ at build time) */
    const legal =
    ...
    if (!legal) {
      errs.push(
        `asset must be under /img/wines/${slug}/ or /api/admin/assets/${slug}/file/`,
      );
    }
  }
