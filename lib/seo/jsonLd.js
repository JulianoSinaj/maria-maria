...
/* `Organization` — weder `LocalBusiness` noch `OnlineStore` (Homepage-Brief
   §7, verbindlich). LocalBusiness verspricht Öffnungszeiten und einen
   Kartenpunkt, an dem man Wein kaufen kann — am Firmensitz gibt es kein
   Ladengeschäft mit Laufkundschaft, wer hinführe, stünde vor einer Tür.
   OnlineStore behauptete einen eigenen Verkaufskanal, den es nicht gibt:
   Verkauft wird im externen Partner-Shop (lib/shop/config.js). Die
   Organisation ist, was übrig bleibt und stimmt: das Unternehmen hinter der
   Marke, mit Impressumsdaten, Profilen und der Marke selbst (`brand`). */
export function organizationNode({ description } = {}) {
  return {
    ...
    /* Kein telephone-Feld: BUSINESS führt bewusst keine Nummer (siehe
       lib/site.js), und eine falsche wäre im Wissensgraph teurer als keine. */
    contactPoint: {
      "@type": "ContactPoint",
...
/* Marke und Unternehmen sind zwei Dinge: „Maria Maria" ist der Name auf dem
   Etikett, die Maria Maria Wines GmbH die Firma dahinter. Bis hierher trug
   nur jeder Produkt-Knoten eine anonyme `brand: { name }` — neun namensgleiche
   Marken ohne Verbindung. Der eine Knoten mit @id macht daraus EINE Marke,
   an der Organisation (`brand`) und Produkte hängen können. Slogan und Logo
   sind dieselben wie am Unternehmen; das Logo wird per @id zitiert. */
...
/* Die drei Knoten, an denen alles andere hängt — Unternehmen, Marke,
   Website. Sie standen bis August 2026 als eigener JSON-LD-Block im
   Root-Layout und die Seite lieferte einen zweiten dazu. Der Homepage-Brief
   (§7) verlangt EINEN serverseitig gerenderten Graphen je Seite; ein Layout
   kann aber nicht wissen, welche Seite es umschließt. Deshalb holt sich
   jede Seite die drei Knoten hier ab und stellt sie an den Anfang ihres
   eigenen `graph()`-Aufrufs — dieselben @ids auf jeder Seite, nur eben im
   selben Block wie WebPage, Produkt und FAQ. */
export function siteNodes({ locale = DEFAULT_LOCALE, description } = {}) {
  return [organizationNode({ description }), brandNode(), websiteNode(locale)];
}
...
/* Der Verkäufer ist nicht automatisch Maria Maria.

   Solange SHOP_ENABLED = false ist, nimmt die eigene Kasse kein Geld an
   (lib/shop/config.js) — gekauft wird bei Terra Vera. Ein `seller`, der auf
   die eigene Organisation zeigt, behauptete dann genau das, was der
   Homepage-Brief §3 im sichtbaren Text ausdrücklich verbietet: Direktversand
   durch Maria Maria. Strukturierte Daten sind dabei nicht die mildere
   Variante, sondern die schärfere — sie landen maschinenlesbar in
   Merchant-Listings, wo eine falsche Händlerangabe eine Richtlinien-
   verletzung ist und nicht bloß eine ungenaue Formulierung.

   Der Knoten trägt eine eigene @id, damit alle neun Angebote auf DENSELBEN
   Händler zeigen statt auf neun namensgleiche Kopien. */
export const PARTNER_SELLER_ID = `${PARTNER_SHOP_ORIGIN}/#organization`;

function partnerSellerNode() {
  return {
    "@type": "Organization",
    "@id": PARTNER_SELLER_ID,
    name: PARTNER_SHOP_NAME,
    url: `${PARTNER_SHOP_ORIGIN}/`,
  };
}

/* `sku` ist der Slug aus components/data.js — derselbe Schlüssel, unter dem
   lib/shop/config.js die Partner-Produktseite führt. */
export function offerNode({ url, price, availability = "InStock", sku = null }) {
  /* Wohin der Kauf führt. Ein `Offer.url`, der auf die eigene Weinseite
     zeigt, schickt den Käufer im Rich Result auf eine Seite ohne Kasse. */
  const buyUrl = SHOP_ENABLED ? url : externalShopUrl(sku);
  ...
  return {
    ...
    seller: SHOP_ENABLED ? { "@id": ORG_ID } : partnerSellerNode(),
    /* Versand- und Rückgabekonditionen NUR im eigenen Shop.

       COMMERCE (lib/site.js) und die 14-Tage-Frist stammen aus den eigenen
       AGB — sie gelten für einen Kauf bei Maria Maria. Wer bei Terra Vera
       bestellt, schließt den Vertrag mit Terra Vera und bekommt deren
       Konditionen. Diese Zahlen an ein fremdes Angebot zu hängen, wäre eine
       Zusicherung im Namen eines Dritten: falsch, sobald Terra Vera etwas
       anderes berechnet, und nicht überprüfbar. Weglassen ist korrekt —
       Google behandelt fehlende Angaben als „unbekannt", nicht als Fehler. */
    ...(SHOP_ENABLED
      ? {
...
export function productNode({
  ...
  return {
    ...
    ...(properties.length
      ...
      : null),
    offers: offerNode({ url, price, availability, sku }),
...
/* Die vier Anliegen der Kontaktseite als Service-Knoten.

   Warum überhaupt? Der Graph beschrieb bis hierher, WER Maria Maria ist
   (Organisation) und WAS verkauft wird (neun Produkte) — aber nicht, was das
   Haus über die Flasche hinaus anbietet: Sortimentsberatung für die
   Gastronomie, Belieferung des Fachhandels, Weinauswahl für Veranstaltungen,
   Verkostungen. Genau das sind die vier Kacheln der Kontaktseite, und genau
   danach wird gesucht („Weinberatung Gastronomie Düsseldorf", „Weinverkostung
   Firmenevent NRW"). Ohne Markup muss eine Antwortmaschine diese Angebote aus
   Fließtext erraten; mit ihm bekommt sie Name, Beschreibung, Anbieter,
   Einzugsgebiet und den Weg zur Anfrage als Paar geliefert.

   Regel 1 dieser Datei gilt unverändert: Name und Beschreibung stammen Wort
   für Wort aus den sichtbaren Karten (content/<sprache>/kontakt.js). Hier
   steht nichts, was ein Besucher nicht liest.

   Die Knoten hängen über `provider` am Organisationsknoten und über
   `availableChannel.serviceUrl` an der Kontaktseite — sie stehen also nicht
   beziehungslos im Graphen, sondern zwischen Anbieter und Anfrageweg.

   `slug` ist bewusst der technische Anliegen-Wert (gastronomie_feinkost …)
   und nicht die Überschrift: Die @id einer Dienstleistung soll dieselbe
   bleiben, wenn die Redaktion die Kachel umformuliert. */

/* Das Einzugsgebiet, das die Seite selbst nennt — die Dachzeile über der H1
   lautet „Weinberatung für Düsseldorf & NRW", und Schritt 2 des Ablaufs
   verspricht die Verkostung „in Düsseldorf und Umgebung". Deutschlandweit
   versendet wird trotzdem (COMMERCE.shipsTo); das steht im Offer-Knoten der
   Produkte, wo es hingehört. `areaServed` beschreibt hier die persönliche
   Leistung, nicht den Paketweg. */
const SERVICE_AREA = [
  { "@type": "City", name: "Düsseldorf" },
  { "@type": "AdministrativeArea", name: "Nordrhein-Westfalen" },
];

export function serviceNode({ url, slug, name, description = null, channelUrl = null, channelName = null }) {
  if (!url || !slug || !name) return null;

  return {
    "@type": "Service",
    "@id": nodeId(url, `service-${slug}`),
    name,
    ...(description ? { description } : null),
    provider: { "@id": ORG_ID },
    ...
    ...(channelUrl
      ? {
          availableChannel: {
            "@type": "ServiceChannel",
            ...(channelName ? { name: channelName } : null),
...
/* Bündelt Knoten zu einem @graph. Eine fertige Seite trägt genau EINEN
   Block: `siteNodes()` (Unternehmen, Marke, Website) gefolgt von den
   Knoten der Seite selbst. Alle Verweise per @id — `isPartOf`, `about`,
   `publisher`, `seller`, `brand` — zeigen damit auf Knoten im selben
   Block; kein Parser muss zwei Scripts eines Dokuments zusammenführen, um
   sie aufzulösen. Verschachtelte Arrays (siteNodes) werden geglättet,
   `null` (leere FAQ, fehlende Breadcrumb) fällt heraus. */
