...
/* `skipPaths` holds the web paths of files the scanner should treat as
   invisible. Exactly one thing goes in there today: the WebP/AVIF widths that
   lib/media/derivatives.js writes beside an upload. They are real files and
   they are served, but they are not assets — listing them would turn one
   uploaded photo into eleven rows and make every count meaningless.

   Passed in rather than detected here, and recorded rather than guessed from
   the filename: an editor may well upload a photo that is honestly called
   "terrasse-640.webp", and it would be listed. The caller (the gallery route)
   knows which files it generated. */
export async function scanGallery({ skipPaths } = {}) {
  const out = [];
  ...
  return skipPaths?.size ? out.filter((asset) => !skipPaths.has(asset.path)) : out;
}
