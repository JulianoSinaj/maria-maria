...
/* Die zwei Seelen hinter dem Namen — als Etiketten-Paar gestaltet:
   ...
   Die beiden Karten liegen leicht schräg und gegeneinander versetzt auf der
   Seite — wie zwei von Hand aufgelegte Etiketten. Beim Hover richtet sich die
   angesteuerte Karte auf, während die andere einen Schritt zurücktritt: das
   Paar bleibt in Bewegung, ohne dass etwas im Layout springt. */
...
const SOULS = [
  {
    ...
    dark: true,
    /* Ruhelage: Neigung, Höhenversatz und Anflugrichtung beim Einblenden */
    tilt: -1.3,
    from: -38,
...
function SoulCard({ soul, active, dimmed, onEnter, onLeave }) {
  const reduced = useReducedMotion();
  const { dark } = soul;

  return (
    /* Der äußere Rahmen trägt den responsiven Höhenversatz als Klasse — Motion
       schreibt `transform` inline und würde eine Tailwind-Translate schlucken. */
    <div className={`h-full ${soul.offset}`}>
      <motion.div
        className="h-full"
        variants={
          reduced
      ...
      >
        <motion.div
          className="h-full"
          /* Ruhe-Neigung sofort setzen, nicht aus der Null herandrehen */
          initial={false}
          animate={
            reduced
              ? undefined
              : { rotate: active ? 0 : soul.tilt, opacity: dimmed ? 0.78 : 1, scale: dimmed ? 0.978 : 1 }
          }
          transition={{ type: "spring", stiffness: 200, damping: 24 }}
          onPointerEnter={(e) => e.pointerType !== "touch" && onEnter()}
          onPointerLeave={(e) => e.pointerType !== "touch" && onLeave()}
        >
          <TiltCard className="group h-full" max={7} radius="rounded-card">
            <div
            ...
            >
              ...
              <div
                className={`relative flex h-full flex-col items-center rounded-[1.125rem] border px-6 py-9 text-center transition-colors duration-500 sm:px-7 ${
                  dark
              ...
              >
                ...
                {/* Auftakt statt Ornament: eine Haarlinie, die sich beim
                    Einblenden von oben herabzieht und beim Hover nachwächst.
                    Eigene whileInView-Steuerung — innerhalb von TiltCard gilt
                    dessen rest/hover-Variantenkontext, nicht der der Sektion. */}
                <motion.span
                  aria-hidden="true"
                ...
                >
                  {/* getrennte Ebene für den Hover — Motion schreibt oben ein
                      inline `transform`, das eine Tailwind-Skalierung überschriebe */}
                  <span
                    className={`block h-7 w-px origin-top transition-transform duration-500 ease-out-expo group-hover:scale-y-[1.35] ${
                      dark ? "bg-champagne/60" : "bg-champagne/70"
                    }`}
                  />
                </motion.span>
