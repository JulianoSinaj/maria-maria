/* End-to-end check of the Anfragen inbox against a running server:
     BASE_URL=http://localhost:3311 ADMIN_SESSION_SECRET=… node lib/inquiries/__tests__/api.test.mjs
   ...
   Against a dev server nothing more is needed — the middleware leaves the
   backoffice open without a signing key. Against a production build pass
   the server's ADMIN_SESSION_SECRET (or ADMIN_PASSWORD, its fallback): the
   script then mints an owner session the way lib/admin/session.js does and
   sends it as the mm_admin cookie.
...
/* <expiry>.<base64url payload>.<base64url HMAC-SHA256> — see createSession() */
function mintSession(secret) {
  const b64url = (buf) => Buffer.from(buf).toString("base64url");
  const expires = Math.floor(Date.now() / 1000) + 60 * 60;
  const payload = b64url(JSON.stringify({ e: null, n: "API-Test", r: "owner", v: "dev" }));
  const message = `${expires}.${payload}`;
  return `${message}.${b64url(createHmac("sha256", secret).update(message).digest())}`;
}

const SECRET = process.env.ADMIN_SESSION_SECRET || process.env.ADMIN_PASSWORD || "";
const AUTH = SECRET ? { Cookie: `mm_admin=${mintSession(SECRET)}` } : {};
...
console.log("\nCSV EXPORT:");
/* raw bytes: fetch's text() silently drops a leading BOM, and the BOM is
   exactly what this check is about */
const csvRes = await fetch(`${A}?format=csv&search=${encodeURIComponent(company)}`, { headers: AUTH });
const csvBytes = new Uint8Array(await csvRes.arrayBuffer());
ok(csvRes.status === 200 && /^text\/csv/.test(csvRes.headers.get("content-type") ?? ""),
   `content-type ${csvRes.headers.get("content-type")}`);
ok(/attachment; filename="anfragen-\d{4}-\d{2}-\d{2}\.csv"/.test(csvRes.headers.get("content-disposition") ?? ""),
   `content-disposition ${csvRes.headers.get("content-disposition")}`);
ok(csvBytes[0] === 0xef && csvBytes[1] === 0xbb && csvBytes[2] === 0xbf, "starts with a UTF-8 BOM (Excel)");
const lines = Buffer.from(csvBytes.subarray(3)).toString("utf8").replace(/\r\n$/, "").split("\r\n");
