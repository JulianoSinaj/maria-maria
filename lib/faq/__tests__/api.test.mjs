...
ok(
  groups.find((g) => g.key === "kontakt")?.nested === true &&
    groups.find((g) => g.key === "home")?.nested === false,
  "kontakt is clustered, home is flat",
);
ok(
  groups.find((g) => g.key === "wine:lugana")?.name === "Lugana DOC",
  "wine groups carry the wine's name",
);
ok(r.b.data.count === 105, `${r.b.data.count} seeded questions (60 page + 45 wine)`);
ok(
...
ok(r.s === 200 && r.b.data.length === 13, `kontakt has ${r.b.data.length} questions`);
ok(
  r.b.data[0].subgroup === "allgemein" && r.b.data.at(-1).subgroup === "shop",
  "list follows the page's cluster order, then position",
);
...
r = await POST({ id: "kontakt-erreichen", group: "kontakt", subgroup: "shop", text: text("Q", "A") });
ok(r.s === 409, "duplicate id → 409");
r = await POST({ id: "test-unknown-group", group: "nirgends", text: text("Q", "A") });
ok(r.s === 422, "unknown group → 422");
r = await POST({ id: "test-no-cluster", group: "kontakt", text: text("Q", "A") });
...
ok(r.b.data.order === 2, `it lands at the end of its cluster (order ${r.b.data.order})`);
ok(
...
const before = (await J(`${B}?group=kontakt`)).b.data
  .filter((i) => i.subgroup === "shop")
  .map((i) => i.id);
r = await PATCH({ action: "reorder", group: "kontakt", subgroup: "shop", ids: [...before].reverse() });
ok(r.s === 200, "reorder → 200");
const after = (await J(`${B}?group=kontakt`)).b.data
  .filter((i) => i.subgroup === "shop")
  .map((i) => i.id);
ok(JSON.stringify(after) === JSON.stringify([...before].reverse()), "the cluster is in the new order");
await PATCH({ action: "reorder", group: "kontakt", subgroup: "shop", ids: before });
...
r = await ITEM("test-terra-vera", {
  method: "PATCH",
  body: JSON.stringify({ action: "rename", id: "kontakt-erreichen", force: true }),
});
...
console.log("\nCLUSTERS:");
r = await PATCH({ action: "subgroup", group: "kontakt", key: "test-cluster", label: { de: "Test" } });
ok(r.s === 200 && r.b.data.some((s) => s.key === "test-cluster"), "create a cluster");
r = await J(`${B}?group=kontakt&subgroup=test-cluster`, { method: "DELETE" });
ok(r.s === 200, "an empty cluster can go");
r = await J(`${B}?group=kontakt&subgroup=shop`, { method: "DELETE" });
