...
/* NOT folded into null: that would hide it from validateDocument() and a
   mistyped date would silently clear the stamp instead of being refused. */
ok(sanitizeDocument({ title: "T", sections: [], reviewedAt: "gestern" }).reviewedAt === "gestern", "an unparseable review date survives sanitising, so validation can refuse it");
ok(validateDocument({ ...doc(), reviewedAt: "gestern" }).some((e) => e.includes("reviewedAt")), "…and validation does refuse it");
