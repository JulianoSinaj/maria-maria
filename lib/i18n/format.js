...
/* Zähl-Einheit in der richtigen Form.

   Deutsch, Italienisch und Englisch kennen zwei Formen (1 / viele) —
   Tschechisch drei: 1 víno, 2–4 vína, ab 5 vín. Ein `count === 1 ? one : many`
   trifft im Tschechischen deshalb bei jeder Zahl von 2 bis 4 daneben; die
   Kollektionsseite zeigte gefiltert „4 vín" statt „4 vína".

   `few` ist optional: Wo eine Sprache die mittlere Form nicht kennt, fehlt der
   Schlüssel und die Funktion fällt auf `many` zurück — die deutsche, ital. und
   engl. Ausgabe bleibt damit unverändert. Dasselbe gilt für `one`. */
export function pickPlural(forms = {}, count) {
...
/* Die Weinart-Zähler von Kollektion und Shop — gleiche Regel, eigene
   Schlüsselnamen (wineOne/wineFew/wineMany liegen in catalogue.filters). */
export function pluralUnit(labels = {}, count) {
  return pickPlural(
    { one: labels.wineOne, few: labels.wineFew, many: labels.wineMany },
    count
  );
}
