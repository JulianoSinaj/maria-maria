...
import { WINES, detailHref } from "../../components/data.js";
/* Not INTERVIEW_SLUGS from interviewRegistry.js: since the backoffice's
   magazine editor, that list is an async lookup against a store on disk
   (fs) — exactly the kind of server-only dependency this file must never
   carry, because the beacon ships it to the browser (see the header above
   splitPublicPath). A published interview's slug is validated server-side
   by the route itself (unknown slugs still 404 there); here a well-formed
   slug segment is enough to count the visit under its own page key instead
   of lumping it into "other". */
import { SLUG_RE as INTERVIEW_SLUG_RE } from "../interviews/schema.js";
import {
...
export function pageKey(path) {
  ...
  if (interview && isInterviewSlug(interview[1])) return path;
  return PAGE_OTHER;
}

export const winePagePath = (slug) => `/unsere-weine/${slug}`;
export const interviewPagePath = (slug) => `/magazin/interviews/${slug}`;
export const isInterviewSlug = (slug) => typeof slug === "string" && INTERVIEW_SLUG_RE.test(slug);
