...
/* The session token is minted here rather than imported from
   lib/admin/session.js: that module imports "./roles" without a file
   extension, which Node's own ESM loader refuses outside the bundler.
   Shape: `<expiry>.<base64url payload>.<HMAC over the first two>`. */
const b64url = (s) =>
...
function mintSession(secret) {
  ...
  const payload = b64url(
    JSON.stringify({ e: "test@maria-maria.de", n: "Test", r: "owner", v: "password" }),
  );
  ...
  const sig = createHmac("sha256", secret)
    ...
    .replace(/\//g, "_")
    .replace(/=+$/, "");
  return `mm_admin=${message}.${sig}`;
}

const secret = process.env.ADMIN_SESSION_SECRET || process.env.ADMIN_PASSWORD || "";
const H = { "Content-Type": "application/json" };
if (secret) H.Cookie = mintSession(secret);
...
/* The archive is append-only and the cleanup above RECORDS its reset rather
   than erasing anything, so a second run starts with a non-empty history.
   The assertions below therefore measure the DELTA, never an absolute count. */
const revisionsBefore = (await GET("agb", "de")).b.data.revisions.length;
...
ok(r.b.data.revisions.length === revisionsBefore + 1 && r.b.data.revisions[0].action === "edit",
   "exactly one revision was added, action “edit”");
