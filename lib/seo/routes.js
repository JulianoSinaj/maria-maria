...
import { listInterviewSlugs, interviewPath } from "@/components/magazin/interviewRegistry";
...
const ALL_STATIC_ROUTES = [
  ...
  /* 0.5/„yearly" stammte aus der Zeit, als /kontakt ein Formular mit einer
     Adresse darunter war. Seit dem Relaunch ist es die Lead-Strecke für
     Gastronomie, Handel, Events und Verkostungen — die einzige Seite der
     Domain, auf der aus einem Besuch eine Anfrage wird, und sie trägt eigene
     Suchbegriffe („Weinberatung Gastronomie", „Weinverkostung Düsseldorf").
     Sie gehört damit vor die Markenerzählung, nicht hinter sie. */
...
/* Die redaktionellen Gespräche. Sie tragen dieselbe Gewichtung wie das
   ...
   Eine Abfrage, keine Konstante mehr: Die Liste kommt aus dem Interview-
   Register (Sprachdateien + Speicher der Redaktion) und zählt nur
   Veröffentlichtes. Ein Entwurf hat keine Adresse, die ein Crawler kennen
   sollte. */
...
export async function seoRoutes() {
  return [...STATIC_ROUTES, ...WINE_ROUTES, ...(await interviewRoutes())];
}
