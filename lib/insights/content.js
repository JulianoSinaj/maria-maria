...
/* An interview is a draft when the dictionary says so, and it is
   "incomplete" when it is published without a date — the article renders,
   but its structured data carries no datePublished (the route drops the
   field rather than inventing one), which is the difference between an
   article Google can date and one it cannot. */
async function interviewStatus() {
  const dicts = Object.fromEntries(
    await Promise.all(LOCALES.map(async (l) => [l, await getDictionary(l)])),
  );

  return INTERVIEW_SLUGS.map((slug) => {
    const perLocale = LOCALES.map((locale) => {
      const item = (dicts[locale]?.interviews?.items ?? []).find((i) => i.slug === slug);
      return {
        locale,
        present: Boolean(item),
        draft: Boolean(item?.draft),
        dated: Boolean(item?.byline?.date),
      };
    });

    const de = perLocale.find((p) => p.locale === DEFAULT_LOCALE);
    return {
      slug,
      title: dicts[DEFAULT_LOCALE]?.interviews?.items?.find((i) => i.slug === slug)?.name ?? slug,
      draft: Boolean(de?.draft),
      missingLocales: perLocale.filter((p) => !p.present).map((p) => p.locale),
      undatedLocales: perLocale.filter((p) => p.present && !p.dated).map((p) => p.locale),
      perLocale,
    };
  });
}
