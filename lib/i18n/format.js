...
/* Zähl-Einheit in der richtigen Form.
   ...
   engl. Ausgabe bleibt damit unverändert. */
export function pluralUnit(labels = {}, count) {
  const n = Math.abs(Number(count));
  if (n === 1 && labels.wineOne) return labels.wineOne;
  if (n >= 2 && n <= 4 && labels.wineFew) return labels.wineFew;
  return labels.wineMany ?? labels.wineOne ?? "";
}
