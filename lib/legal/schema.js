...
export function sanitizeDocument(input) {
  ...
  /* null is a meaningful value here — "this document has no review stamp" —
     and has to survive, so it is not folded into undefined */
  if (input?.reviewedAt !== undefined) {
    out.reviewedAt = isIsoDay(input.reviewedAt) ? input.reviewedAt : null;
  }
