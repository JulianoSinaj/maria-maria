...
import { LOCALES, DEFAULT_LOCALE, LOCALE_META } from "../i18n/config.js";
import { listInterviews } from "../interviews/store.js";
import { INTERVIEW_LOCALES } from "../interviews/schema.js";
import { WINE_SLUGS } from "../../components/weine/wineRegistry.js";
/* The rules for "is this string still missing" live next door, on their own,
   so they can be tested without loading four dictionaries — see
   lib/insights/__tests__/content.test.mjs. */
...
/* How many individual missing keys to name per language. The count is
   exact; the list is a sample, because "412 fehlende Schlüssel" rendered as
   412 lines is a wall, not a report. */
const SAMPLE = 8;
...
async function translationGaps() {
  ...
  for (const locale of others) {
    ...
    for (const path of paths) {
      if (isTranslated(readPath(dict, path), locale, path)) continue;
      missing.push(path);
